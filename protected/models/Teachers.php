...
class Teachers extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('name, surname, midname, phone', 'length', 'max'=>50),
			array('image', 'length', 'max'=>100),
            array('image_file','file', 'types'=>'jpg, gif, png','allowEmpty' => true),
            // The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, surname, midname, image, phone', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
            'courses'=>array(self::HAS_MANY,'Courses','teacher_id'),
		);
	}
    ...
    public static function getAll(){
        $teachers = self::model()->findAll();
        $result = array();
        foreach($teachers as $teacher){
            $result['id'] = $teacher->id;
            $result['fullName'] = $teacher->surname . " " . $teacher->name . " " . $teacher->midname;
        }
        return CHtml::listData($teachers,'id','fullName');
    }
